...
MODEL                    = SingleTrackCANoe
MODULES                  = rt_logging.c SingleTrackCANoe_capi.c SingleTrackCANoe_data.c SingleTrackCANoe_dynamap.c rtGetInf.c rtGetNaN.c rt_look.c rt_look1d.c rt_nonfinite.c rt_zcfcn.c rt_logging_mmi.c rtw_modelmap_utils.c vdy_rt_work.c
MAKEFILE                 = SingleTrackCANoe.mk
...
S_FUNCTIONS              = 
S_FUNCTIONS_LIB          = C:\PROGRA~1\VEE7EA~1\lib\win64\DYNA4_~2.LIB
SOLVER                   = 
NUMST                    = 2
TID01EQ                  = 1
NCSTATES                 = 0
BUILDARGS                =  MAT_FILE=1 GENERATE_MODEL_DESCRIPTION="off" MIP_CONFIG_XCP_TIMESTAMPS_DISABLE="off" DEBUG_BUILD=0 HAS_WES64_BUILD="off" HAS_WES_BUILD="on" HAS_ERT_BUILD="off" TARGET_ARCHITECTURE="Standard 64 Bit Real-Time DLL" GENERATE_VMODULE="on" COMBINE_OUTPUT_UPDATE_FCNS=0 INCLUDE_MDL_TERMINATE_FCN=1 MULTI_INSTANCE_CODE=0 OPTS="-DTID01EQ=1"
MULTITASKING             = 0
EXT_MODE             = 0
...
ENABLE_SIGNALANALYSIS    = 0
ENABLE_PARAMETERIZATION_REFMODELS = 0
SHARED_SRC               = 
SHARED_SRC_DIR           = 
SHARED_BIN_DIR           = 
SHARED_LIB               = 
...
CANOE_SRC                = C:/PROGRA~1/VEE7EA~1/3rdParty/CANOEM~1/c/canoe
CANOE_BIN                = C:/PROGRA~1/VEE7EA~1/3rdParty/CANOEM~1/bin/canoe
SAMPLE_TIMES             = [0.001,0,0]
TASK_OVERRUN_STRATEGY    = 2
MAX_QUEUED_TASKS         = 5
ENABLE_PARAMETERIZATION_EXCLUSION = 1

#--------------------------- Model and reference models -----------------------
MODELREFS                 = 
MODELLIB                  = 
MODELREF_LINK_LIBS        = 
MODELREF_LINK_RSPFILE     = SingleTrackCANoe_ref.rsp
MODELREF_INC_PATH         = 
RELATIVE_PATH_TO_ANCHOR   = ..
MODELREF_TARGET_TYPE      = NONE

REQUIRED_VIA_VERSION      = 9
...
INCLUDE = $(WINSDK_71A)include;$(INCLUDE)
FILENAME= $(MODEL)
!if $(BUILD_ERT) == 1
WEC_PATH= $(CANOE_SRC)\WEC
!else
!  if "$(TARGET_ARCH)" == "x86"
PATH = $(WINSDK_71A)bin;$(PATH)
LIB = $(WINSDK_71A)lib;$(LIB)
!  else
PATH = $(WINSDK_71A)bin\x64;$(PATH)
LIB = $(WINSDK_71A)lib\x64;$(LIB)
!  endif
!endif
...
!if $(BUILD_ERT) == 1
DEF  = $(CANOE_SRC)\private\vtp_ert_apml.def
!else
...
!      if "$(TARGET_ARCH)" == "x86"
!        if $(DEBUG_BUILD) == 1 
CANOE_LIBS = $(CANOE_SRC)\private\lib\CANoeEmu_VC120_D.lib
!        else
CANOE_LIBS = $(CANOE_SRC)\private\lib\CANoeEmu_VC120_R.lib
!        endif
!      else
!        if $(DEBUG_BUILD) == 1
CANOE_LIBS = $(CANOE_SRC)\private\lib\CANoeEmu_VC120_64D.lib
!        else
CANOE_LIBS = $(CANOE_SRC)\private\lib\CANoeEmu_VC120_64R.lib
!        endif
!      endif
...
!      endif
!    else
!      error "Multitasking is only supported for compilers VC12.0, VC15.0, VC16.0, VC17.0"
CANOE_LIBS = 
!    endif
!  else # Build ERT Multitasking.
!    if $(DEBUG_BUILD) == 1
CANOE_LIBS = $(CANOE_SRC)\private\lib\VtpErt_VC12.0_D.lib
!    else
CANOE_LIBS = $(CANOE_SRC)\private\lib\VtpErt_VC12.0_R.lib
!    endif
!  endif
!else #Singletasking
!  if $(BUILD_ERT) == 0
CANOE_LIBS = 
!  else # Build ERT Singletasking.
!    if $(DEBUG_BUILD) == 1
CANOE_LIBS = $(CANOE_SRC)\private\lib\VtpErt_VC12.0_D.lib
!    else
CANOE_LIBS = $(CANOE_SRC)\private\lib\VtpErt_VC12.0_R.lib
...
!if $(MULTITASKING) == 1 && $(BUILD_ERT) == 0
          /D TASK_OVERRUN_STRATEGY=$(TASK_OVERRUN_STRATEGY) /D MAX_QUEUED_TASKS=$(MAX_QUEUED_TASKS) \
!endif
!if $(BUILD_ERT) == 1
# V_OS_WEC for via.h
          /D "V_ERT_RTP" /D "V_ERT_WEC" /D "V_OS_WEC" /D "_WIN32_WCE=0x800" /D "_MSC_VER=1700" /D "x86" /D "_X86_" /D "_USESTL" \
          /D "_UNICODE" /D "UNICODE" /D "_WINDLL" /D "__VRTP_RELEASE_BUILD_" \
!endif
!if "$(TARGET_ARCH)" == "x64"
          /D "V_OS_64BIT" \
!endif
...
!if $(ENABLE_PARAMETERIZATION_EXCLUSION) > 0 && $(BUILD_ERT) == 0
...
MATLAB_INCLUDES = $(MATLAB_INCLUDES);C:\Users\USER\DYNA4_9.1\Projects\My_Demo_Basic\work.matlab
MATLAB_INCLUDES = $(MATLAB_INCLUDES);C:\Users\USER\DYNA4_9.1\Projects\My_Demo_Basic\work.matlab\SingleTrackCANoe_cn_rtw
MATLAB_INCLUDES = $(MATLAB_INCLUDES);$(MATLAB_ROOT)\extern\include
MATLAB_INCLUDES = $(MATLAB_INCLUDES);$(MATLAB_ROOT)\simulink\include
MATLAB_INCLUDES = $(MATLAB_INCLUDES);$(MATLAB_ROOT)\rtw\c\src
MATLAB_INCLUDES = $(MATLAB_INCLUDES);$(MATLAB_ROOT)\rtw\c\src\ext_mode\common
MATLAB_INCLUDES = $(MATLAB_INCLUDES);C:\PROGRA~1\VEE7EA~1\include
...
WEC_LIBS = $(WEC_PATH)\Lib\ 
!if $(BUILD_ERT) == 1 
WEC_SYS_LIBS = coredll.lib
WEC_SYS_LIBS_R = msvcrt.lib
WEC_SYS_LIBS_D = msvcrtd.lib
!  if $(DEBUG_BUILD) == 1 
WEC_SYS_LIBS = $(WEC_SYS_LIBS) $(WEC_SYS_LIBS_D)
!  else 
WEC_SYS_LIBS = $(WEC_SYS_LIBS) $(WEC_SYS_LIBS_R)
!  endif 
!else 
WEC_SYS_LIBS = 
!endif
...
#----------------------------- Defines ----------------------------------------
MYOPTS = /EHsc /Fp"$(FILENAME).pch" /FD /W3 /nologo /c /D "_CRT_SECURE_NO_DEPRECATE"
!if $(BUILD_ERT) == 1
MYOPTS   = $(MYOPTS)\
  /Zi /Gm- /fp:precise /Zc:forScope \
  /analyze- /Zc:wchar_t /WX- /Gd $(OPTS) \
  /arch:SSE           # Difference between VS2012 and VS2013 does not allow usage of SSE2 conversion functions, but everything seems to work.
!else
MYOPTS   = $(MYOPTS)\
  /D "WIN32" /D "_WINDOWS" /D "_MBCS" /D "_USRDLL" $(OPTS) \
!endif
...
!  if $(DEBUG_BUILD) == 1
OPT_OPTS = $(OPT_OPTS) /Zi /MDd /D "_DEBUG" /D "DEBUG"
!  else
OPT_OPTS = $(DEFAULT_OPT_OPTS) $(OPT_OPTS) /GL /sdl /Oy- /MD /D "__V_NDEBUG" /D "_NDEBUG" /D "NDEBUG"
!  endif
!else
# Optimization / Debugging Options
# Use MT resp. MTd instead of MD, because MD requires msvcrXX.dll to be present at runtime, which is not always available.
!  if $(DEBUG_BUILD) == 1
OPT_OPTS = $(OPT_OPTS) /Zi /MDd /D "_DEBUG"
!  else
OPT_OPTS = $(DEFAULT_OPT_OPTS) $(OPT_OPTS) /MD /D "NDEBUG" /Zi
...
!if $(HAS_CAPL_CALL) == 1 #|| $(BUILD_ERT) == 1
...
!if $(BUILD_ERT) == 0
CPP_REQ_DEFINES = $(CPP_REQ_DEFINES) /D _USING_V110_SDK71_ 
!endif
...
LDFLAGS = /DEBUG /nologo /DLL $(DEFFLAGS) \
          ...
          $(EXT_LIB)
!if $(BUILD_ERT) == 1
LDFLAGS  =  $(LDFLAGS) /SAFESEH /NODEFAULTLIB /MANIFEST /MANIFESTUAC:NO /NXCOMPAT /MACHINE:X86 \
            /LIBPATH:$(WEC_LIBS) /TLBID:1 /SUBSYSTEM:WindowsCE  /ERRORREPORT:PROMPT \
            /LTCG /DYNAMICBASE /VERBOSE:LIB 
!else
LDFLAGS  =  $(LDFLAGS) /machine:$(TARGET_ARCH) /INCREMENTAL:NO /OPT:REF /OPT:ICF /IGNORE:4099 \
            ws2_32.lib Advapi32.lib Shlwapi.lib
!endif

RCFLAGS = /D  MATLABIFVERSION=\""$(CANOEMATLABIF_VER)"\"
!if $(BUILD_ERT) == 1
RCFLAGS = $(RCFLAGS) /D "V_ERT_WEC"
!else
RCFLAGS = $(RCFLAGS) /D _USING_V110_SDK71_
!endif
!if $(RTT_PE) == 1
...
!if $(BUILD_ERT) == 0 && $(ENABLE_PARAMETERIZATION_EXCLUSION) == 1
...
!if $(BUILD_ERT) == 1
    vtp_ert_apml.cpp \
!elseif $(MULTITASKING) == 1
    cncommfb.cpp vttapml.cpp CANoeEmu_DllMain.cpp FunctionBusServiceVTT.cpp SysvarServiceVTT.cpp ServiceProvider.cpp\
!else
! if $(RTT_PE) == 0
    cncommfb.cpp cncommdo.cpp nlapml.cpp nlapml_common.cpp FunctionBusServiceVIA.cpp SysvarServiceVIA.cpp ServiceProvider.cpp BusServiceVIA.cpp\
! else
    cncommfb.cpp cncommdo.cpp nlapml_common.cpp FunctionBusServiceVIA.cpp SysvarServiceVIA.cpp cncommsharedmem.cpp ServiceProvider.cpp VIANodelayerApi_RTT_PE.cpp BusServiceVIA.cpp SimulationThread.cpp ValueCreatorVIA.cpp\
! endif
!endif
!if $(ENABLE_PARAMETERIZATION) == 1 
    mdlparams.cpp \
...
!endif
             ...
             $(LD) $(LDFLAGS) $(S_FUNCTIONS_LIB) $(SHARED_LIB) $(LIBS) $(MAT_LIBS) $(WEC_SYS_LIBS) $(RESOURCES) @$(CMD_FILE) -out:$@
             @del $(CMD_FILE)
...
{C:\PROGRA~1\VEE7EA~1\include}.c.obj :
  $(CC) $(CFLAGS) $<
{$(MATLAB_ROOT)\rtw\c\src}.c.obj :
  $(CC) $(CFLAGS) $<
...
{C:\PROGRA~1\VEE7EA~1\include}.cpp.obj :
  $(CC) $(CPPFLAGS) $<
